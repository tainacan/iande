<?php
/**
 * Iande_Plugin_Options class.
 *
 * Built settings page for plugin.
 *
 * @category Options
 * @version  1.0.0
 */
class Iande_Plugin_Options {
	...
	function scripts() {
		...
		if ( isset( $_GET['page'] ) && $this->id == $_GET['page'] ) {
			...
			// Plugin Options.
	        wp_enqueue_script('iande-admin', IANDE_BASEURL . 'includes/options/assets/js/admin.js', array('jquery'), null, true);
            wp_enqueue_style('iande-admin', IANDE_BASEURL . 'includes/options/assets/css/admin.css', array(), null, 'all');
            ...
            wp_localize_script(
                ...
                'iandeAdminParams',
                array(
                    'galleryTitle'  => __('Adicionar imagens na galeria', 'iande'),
                    'galleryButton' => __('Adicionar na galeria', 'iande'),
                    'galleryRemove' => __('Remover imagem', 'iande'),
                    'uploadTitle'   => __('Escolher um arquivo', 'iande'),
                    'uploadButton'  => __('Adicionar arquivo', 'iande'),
                )
            );
		}
	}
	...
	public function callback_color( $args ) {
		// Sets color class.
		$args['attributes']['class'] = 'iande-color-field';
	...
	public function callback_upload( $args ) {
		...
		$html = sprintf( '<input type="text" id="%1$s" name="%2$s[%1$s]" value="%3$s" class="regular-text"%5$s /> <input class="button iande-upload-button" id="%1$s-button" type="button" value="%4$s" />', $id, $tab, $current, __( 'Select file', 'iande' ), $this->build_field_attributes( $attrs ) );
	...
	public function callback_image( $args ) {
        ...
        $image = apply_filters('iande_placeholder_image_path', IANDE_BASEURL . 'includes/meta-boxes/assets/images/placeholder.png');
		$html  = '<div class="iande-upload-image">';
	...
	public function callback_image_plupload( $args ) {
		...
		$html = '<div class="iande-gallery-container">';
			$html .= '<ul class="iande-gallery-images">';
				if ( ! empty( $current ) ) {
			...
			$html .= sprintf( '<input type="hidden" id="%1$s" name="%2$s[%1$s]" value="%3$s" class="iande-gallery-field" />', $id, $tab, $current );

			// Adds "adds images in gallery" url.
			$html .= sprintf( '<p class="iande-gallery-add hide-if-no-js"><a href="#">%s</a></p>', __( 'Add images in gallery', 'iande' ) );
		$html .= '</div>';
	...
	public function validate_input( $input ) {
		...
		foreach ( $input as $key => $value ) {
			...
			if ( isset( $input[ $key ] ) ) {
				$output[ $key ] = apply_filters( 'iande_plugin_options_validate_' . $this->id, $value, $key );
			}
