...
function iande_export_button($which) {
    ...
    if ('appointment' === $typenow && 'top' === $which && \current_user_can( 'manage_options' )) {
        ?>
        <input type="submit" name="export_all_posts" id="export_all_posts" class="button button-primary" value="Exportar Agendamentos" />
        <?php
	}
...
function iande_export_appointment() {
    ...
    if (isset($_GET['export_all_posts']) && \is_admin()) {
        ...
        if ($appointments) {
            ...
            $filename = $blogname . '-export-appointment-' . $current_datetime . '.csv';
