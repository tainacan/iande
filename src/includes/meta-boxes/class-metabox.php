...
class Iande_Metabox {
	...
	public function scripts() {
		...
		if ( in_array( $screen->id, $this->get_post_type() ) ) {
			...
			// Metabox.
			wp_enqueue_script( 'iande-admin', IANDE_BASEURL . 'includes/meta-boxes/assets/js/admin.js', array( 'jquery' ), null, true );
			wp_enqueue_style( 'iande-admin', IANDE_BASEURL . 'includes/meta-boxes/assets/css/admin.css', array(), null, 'all' );
	...
	protected function process_fields( $args, $post_id ) {
		...
		switch ( $type ) {
			...
			case 'editor':
				...
				break;
			case 'color':
				$this->field_input( $id, $current, array_merge( array( 'class' => 'iande-color-field' ), $attrs ) );
				break;
			case 'upload':
			...
			default:
				do_action( 'iande_metabox_field_' . $this->id, $type, $id, $current, $options, $attrs );
				break;
		}
	}
	...
	protected function field_upload( $id, $current, $attrs ) {
		echo sprintf( '<input type="text" id="%1$s" name="%1$s" value="%2$s" class="regular-text"%4$s /> <input class="button iande-upload-button" type="button" value="%3$s" />', $id, esc_url( $current ), __( 'Select file', 'iande' ), $this->build_field_attributes( $attrs ) );
	}
	...
	protected function field_image_plupload( $id, $current ) {
		$html = '<div class="iande-gallery-container">';
			...
			$html .= sprintf( '<input type="hidden" class="iande-gallery-field" name="%s" value="%s" />', $id, $current );

			// Adds "adds images in gallery" url.
			$html .= sprintf( '<p class="iande-gallery-add hide-if-no-js"><a href="#">%s</a></p>', __( 'Adicionar imagens na galeria', 'iande' ) );
		$html .= '</div>';
	...
	public function save( $post_id ) {
		...
		foreach ( $this->fields as $field ) {
			...
			if ( ! in_array( $field['type'], array( 'separator', 'title' ) ) ) {
				...
				$new = apply_filters( 'iande_save_metabox_' . $this->id, $value, $name );
